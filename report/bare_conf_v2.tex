...
\section{Discussion} 
\subsection{Performance Metrics} % REVER ?
...
%When considering the work of Paullier et al. \cite{9379914}, where seven different models were tested under the same conditions, to highlight the comparison framework and approach to this type of problem, the models developed and presented in this paper fall well in line with the performance achieved in the literature.
%The Table \ref{tab:atk_results_benchamarkpaper} allows for an almost direct comparation to those results, even thought, the considered work uses a different dataset. QUERIA ADICIONAR ALGO ASSIM NESTE CONTEXTO, OQ ACHAS? PARA FALAR DA TABELA ESTAR MT IDENTICA A DELES TP
%The values are not worth detailing here, as the datasets are not the same, Paullier used the smaller MovieLens 100k dataset, and comparing the FunkSVD models, the one presented in this work shows better metrics, largely due to the dimension of the dataset to better fit the algorithm.
% PODE SE APAGAR ISTO ?
% GOSTAS DO TEXTO ACIMA?
...
% ALTEREI O TEXTO, OQ ACHAS? SE OK, APAGAR O TEU
