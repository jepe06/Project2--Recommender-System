...
%Model 02 - Users and Movies + Genres & Collaborative filtering, && Linear Regression content based (CB-LR) \\[1em] oq achas???????????????????????????
% li que havia um hybrid, tlvz seja onde melhor se encaixa...
...
\begin{figure}[H]
    ...
    \caption{Distribution of movies based on the number of ratings, starting from 1 rating in the first bin.} %tentei alterar um pouco a caption, opinião?
